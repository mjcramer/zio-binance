...
import io.github.mjcramer.binance.spot.api.enums.KlineInterval
import io.github.mjcramer.binance.spot.rest.BinanceSpotClient
import io.github.mjcramer.binance.spot.rest.market.Market
import zio.ZIO
...
trait MarketSpec extends BaseSpec {
  ...
  val timeTolerance = 5000 // milliseconds
  ...
  val marketSuite: Spec[BinanceSpotClient, Throwable] = suite("Market API Tests")(
    ...
    test("Check the servers time is current") {
      ...
      } yield assertTrue(before < after + timeTolerance)
    },
